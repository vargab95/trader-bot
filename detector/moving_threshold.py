...
class MovingThresholdCrossOverDetector(detector.crossover.CrossOverDetector):
    ...
    def check(self, summary: float) -> actions.TradingAction:
        long_term_summary = self.spider.get_technical_summary()

        self.bearish_threshold = self.__saturate(self.original_bearish_threshold - long_term_summary, 1.0)
        self.bullish_threshold = self.__saturate(self.original_bullish_threshold - long_term_summary, 1.0)
