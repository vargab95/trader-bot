...
class CommonMockTest(unittest.TestCase):
    ...
    def test_buy_bear_and_sell_on_same_price(self):
        self.controller.buy(exchange.interface.Market.create_from_string("BEAR-USDT"), 10)
        balance = self.controller.get_balances()
        self.assertEqual(balance["USDT"], 0.0)
        self.assertEqual(balance["BEAR"], 10.0)

        self.controller.sell(exchange.interface.Market.create_from_string("BEAR-USDT"), 10)
        balance = self.controller.get_balances()
    ...
    def test_buy_bear_and_bull_and_sell_on_same_price(self):
        ...
        self.controller.sell(exchange.interface.Market.create_from_string("BEAR-USDT"), 5.0)
        balance = self.controller.get_balances()
        self.assertEqual(balance["USDT"], 90.0)
        self.assertEqual(balance["BEAR"], 0.0)
        self.assertEqual(balance["BULL"], 2.0)

        self.controller.sell(exchange.interface.Market.create_from_string("BULL-USDT"), 2.0)
        balance = self.controller.get_balances()
        self.assertEqual(balance["USDT"], 100.0)
        self.assertEqual(balance["BEAR"], 0.0)
    ...
    def test_buy_bear_and_bull_and_sell_on_same_price_partially(self):
        ...
        self.controller.buy(exchange.interface.Market.create_from_string("BULL-USDT"), 5.0)
        balance = self.controller.get_balances()
    ...
    def test_buy_bull_and_sell_on_lower_price(self):
        ...
        self.controller.sell(exchange.interface.Market.create_from_string("BULL-USDT"), 5.0)
        balance = self.controller.get_balances()
    ...
    def test_buy_too_much(self):
        with self.assertRaises(exchange.interface.InsufficientFundsError):
            self.controller.buy(exchange.interface.Market.create_from_string("BULL-USDT"), 2000.0)

    def test_sell_too_much(self):
        with self.assertRaises(exchange.interface.InsufficientFundsError):
            self.controller.sell(exchange.interface.Market.create_from_string("BULL-USDT"), 2000.0)

    def test_sell_negative(self):
        with self.assertRaises(exchange.interface.ZeroOrNegativeAmountError):
            self.controller.sell(exchange.interface.Market.create_from_string("BULL-USDT"), -1.0)
    ...
    def test_price_history_in_non_real_time(self):
        ...
        with self.assertRaises(NotImplementedError):
            self.controller.get_price_history(descriptor)
    ...
    def test_bet_on_bullish_too_much(self):
        ...
        with self.assertRaises(exchange.interface.InsufficientFundsError):
            self.controller.bet_on_bullish(market, 2000.0)

    def test_bet_on_bearish_too_much(self):
        market = exchange.interface.Market.create_from_string("BEAR-PERP")
        with self.assertRaises(exchange.interface.InsufficientFundsError):
            self.controller.bet_on_bearish(market, 2000.0)
    ...
    def test_close_position_below_liquidation_price(self):
        ...
        with self.assertRaises(exchange.interface.PositionLiquidatedError):
            self.controller.close_position(market)
