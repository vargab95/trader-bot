...
class ResetOnFallingEdgeDetector(detector.interface.DetectorInterface):
    ...
    def check(self, indicator: float) -> detector.common.TradingAction:
        ...
        elif not self._bullish_compare(indicator) and self.current_state == detector.common.CurrentState.BULL:
            result = detector.common.TradingAction.RETURN_TO_BASE_SIGNAL
            self.current_state = detector.common.CurrentState.NONE
        elif not self._bearish_compare(indicator) and self.current_state == detector.common.CurrentState.BEAR:
            result = detector.common.TradingAction.RETURN_TO_BASE_SIGNAL
            self.current_state = detector.common.CurrentState.NONE
